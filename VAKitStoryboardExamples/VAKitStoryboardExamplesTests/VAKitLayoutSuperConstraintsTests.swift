...
class VAKitLayoutSuperConstraintsTests: XCTestCase {
    ...
    func test_toSuperAxisSymmetric_vertical_constraintMultiplier() {
        let multiplier: CGFloat = 42
        var constraintTop: NSLayoutConstraint!
        var constraintBottom: NSLayoutConstraint!
        parentView.addAutolayoutSubview(view)
        view.toSuperAxis(.vertical, symmetric: 1, multiplier: multiplier, configuring: { (constraintTop, constraintBottom) = $0 })
        XCTAssertEqual(multiplier, constraintTop.multiplier)
        XCTAssertEqual(multiplier, constraintBottom.multiplier)
    }
    ...
    func test_toSuperAxisSymmetric_vertical_constraintSafeAreaDefault() {
        var constraintTop: NSLayoutConstraint!
        var constraintBottom: NSLayoutConstraint!
        parentView.addAutolayoutSubview(view)
        view.toSuperAxis(.vertical, symmetric: 1, configuring: { (constraintTop, constraintBottom) = $0 })
    ...
    func test_toSuperAxisSymmetric_horizontal_constraintSafeArea() {
        var constraintLeading: NSLayoutConstraint!
        var constraintTrailing: NSLayoutConstraint!
        parentView.addAutolayoutSubview(view)
        view.toSuperAxis(.horizontal, symmetric: 1, isSafe: true, configuring: { (constraintLeading, constraintTrailing) = $0 })
        XCTAssertEqual(parentView.safeAreaLayoutGuide, (constraintLeading.secondItem as? UILayoutGuide))
        XCTAssertEqual(parentView.safeAreaLayoutGuide, (constraintTrailing.secondItem as? UILayoutGuide))
    }
    ...
    func test_toSuperAnchors_constraintConstantDefault() {
        ...
        constraints.forEach({ XCTAssertTrue($0.constant.isZero) })
    }
    ...
    func test_toSuperAnchors_constraintPriority() {
        ...
        var constraints: [NSLayoutConstraint]!
        parentView.addAutolayoutSubview(view)
        view.toSuper(anchors: .top, .leading, .bottom, priority: priority, configuring: { constraints = $0 })
        constraints.forEach({ constraintPriorityCheck(constraint: $0, priority: priority) })
    }

    func test_toSuperAnchors_constraintRelationDefault() {
        var constraints: [NSLayoutConstraint]!
        parentView.addAutolayoutSubview(view)
        view.toSuper(anchors: .top, .leading, .bottom, configuring: { constraints = $0 })
        constraints.forEach({ XCTAssertEqual(NSLayoutConstraint.Relation.equal, $0.relation) })
    }

    func test_toSuperAnchors_constraintRelation() {
        let relation: NSLayoutConstraint.Relation = .greaterThanOrEqual
        var constraints: [NSLayoutConstraint]!
        parentView.addAutolayoutSubview(view)
        view.toSuper(anchors: .top, .leading, .bottom, relation: relation, configuring: { constraints = $0 })
        constraints.forEach({ XCTAssertEqual(relation, $0.relation) })
    }

    func test_toSuperAnchors_constraintActivation() {
        var constraints: [NSLayoutConstraint]!
        parentView.addAutolayoutSubview(view)
        view.toSuper(anchors: .top, .leading, .bottom, isActive: false, configuring: { constraints = $0 })
